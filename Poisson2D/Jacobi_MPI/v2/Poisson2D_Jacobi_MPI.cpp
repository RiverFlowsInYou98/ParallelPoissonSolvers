...
void exchangeBoundaryData(const MPIEnv &env, const Subdomain &subdomain, std::vector<double> &local_sol,
                          ...
                          MPI_Datatype column_data_type)
{
    if (env.top_neighbor != MPI_PROC_NULL)
    {
        MPI_Sendrecv(&local_sol[idx(0, 0)], subdomain.ny, MPI_DOUBLE, env.top_neighbor, 0,
                     &received_top_boundary_data[0], subdomain.ny, MPI_DOUBLE, env.top_neighbor, 0,
                     env.cart_comm, MPI_STATUS_IGNORE);
    }
    if (env.bottom_neighbor != MPI_PROC_NULL)
    {
        MPI_Sendrecv(&local_sol[idx(subdomain.nx - 1, 0)], subdomain.ny, MPI_DOUBLE, env.bottom_neighbor, 0,
                     &received_bottom_boundary_data[0], subdomain.ny, MPI_DOUBLE, env.bottom_neighbor, 0,
                     env.cart_comm, MPI_STATUS_IGNORE);
    }
    if (env.left_neighbor != MPI_PROC_NULL)
    {
        MPI_Sendrecv(&local_sol[idx(0, 0)], 1, column_data_type, env.left_neighbor, 0,
                     &received_left_boundary_data[0], subdomain.nx, MPI_DOUBLE, env.left_neighbor, 0,
                     env.cart_comm, MPI_STATUS_IGNORE);
    }
    if (env.right_neighbor != MPI_PROC_NULL)
    {
        MPI_Sendrecv(&local_sol[idx(0, subdomain.ny - 1)], 1, column_data_type, env.right_neighbor, 0,
                     &received_right_boundary_data[0], subdomain.nx, MPI_DOUBLE, env.right_neighbor, 0,
                     env.cart_comm, MPI_STATUS_IGNORE);
    }
}
...
int main(int argc, char **argv)
{
    ...
    std::vector<double> sol(subdomain.local_numPoints), sol_old(subdomain.local_numPoints),
        exact_sol(subdomain.local_numPoints), RHS(subdomain.local_numPoints);
    ...
    do
    {
        ...
        exchangeBoundaryData(env, subdomain, sol_old, received_top_boundary_data, received_bottom_boundary_data,
                             received_left_boundary_data, received_right_boundary_data, column_data_type);
        ...
        for (int i = 1; i < subdomain.nx - 1; ++i)
        {
            ...
            {
                sol[idx(i, j)] = (dy * dy * (sol_old[idx(i - 1, j)] + sol_old[idx(i + 1, j)]) +
                                  dx * dx * (sol_old[idx(i, j - 1)] + sol_old[idx(i, j + 1)]) +
        ...
        if (env.top_neighbor != MPI_PROC_NULL)
        {
            for (int j = 0; j < subdomain.ny; ++j)
            {
                ...
                {
                    sol[idx(0, 0)] = (dy * dy * (received_top_boundary_data[0] + sol_old[idx(1, 0)]) +
                                      dx * dx * (received_left_boundary_data[0] + sol_old[idx(0, 1)]) +
                                      dx * dx * dy * dy * RHS[idx(0, 0)]) /
                                     (2 * (dx * dx + dy * dy));
                    maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(0, 0)] - sol_old[idx(0, 0)]));
                }
                else if (j == subdomain.ny - 1 && env.right_neighbor != MPI_PROC_NULL)
                {
                    sol[idx(0, subdomain.ny - 1)] = (dy * dy * (received_top_boundary_data[subdomain.ny - 1] + sol_old[idx(1, subdomain.ny - 1)]) +
                                                     dx * dx * (sol_old[idx(0, subdomain.ny - 2)] + received_right_boundary_data[0]) +
                                                     dx * dx * dy * dy * RHS[idx(0, subdomain.ny - 1)]) /
                                                    (2 * (dx * dx + dy * dy));
                    maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(0, subdomain.ny - 1)] - sol_old[idx(0, subdomain.ny - 1)]));
                }
                else if (j > 0 && j < subdomain.ny - 1)
                {
                    sol[idx(0, j)] = (dy * dy * (received_top_boundary_data[j] + sol_old[idx(1, j)]) +
                                      dx * dx * (sol_old[idx(0, j - 1)] + sol_old[idx(0, j + 1)]) +
                                      dx * dx * dy * dy * RHS[idx(0, j)]) /
                                     (2 * (dx * dx + dy * dy));
                    maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(0, j)] - sol_old[idx(0, j)]));
                }
            }
        }
        ...
        if (env.bottom_neighbor != MPI_PROC_NULL)
        {
            for (int j = 0; j < subdomain.ny; ++j)
            {
                if (j == 0 && env.left_neighbor != MPI_PROC_NULL)
                {
                    sol[idx(subdomain.nx - 1, 0)] = (dy * dy * (sol_old[idx(subdomain.nx - 2, 0)] + received_bottom_boundary_data[0]) +
                                                     dx * dx * (received_left_boundary_data[subdomain.nx - 1] + sol_old[idx(subdomain.nx - 1, 1)]) +
                                                     dx * dx * dy * dy * RHS[idx(subdomain.nx - 1, 0)]) /
                                                    (2 * (dx * dx + dy * dy));
                    maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(subdomain.nx - 1, 0)] - sol_old[idx(subdomain.nx - 1, 0)]));
                }
                else if (j == subdomain.ny - 1 && env.right_neighbor != MPI_PROC_NULL)
                {
                    sol[idx(subdomain.nx - 1, subdomain.ny - 1)] = (dy * dy * (sol_old[idx(subdomain.nx - 2, subdomain.ny - 1)] + received_bottom_boundary_data[subdomain.ny - 1]) +
                                                                    dx * dx * (sol_old[idx(subdomain.nx - 1, subdomain.ny - 2)] + received_right_boundary_data[subdomain.nx - 1]) +
                                                                    dx * dx * dy * dy * RHS[idx(subdomain.nx - 1, subdomain.ny - 1)]) /
                                                                   (2 * (dx * dx + dy * dy));
                    maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(subdomain.nx - 1, subdomain.ny - 1)] - sol_old[idx(subdomain.nx - 1, subdomain.ny - 1)]));
                }
                else if (j > 0 && j < subdomain.ny - 1)
                {
                    sol[idx(subdomain.nx - 1, j)] = (dy * dy * (sol_old[idx(subdomain.nx - 2, j)] + received_bottom_boundary_data[j]) +
                                                     dx * dx * (sol_old[idx(subdomain.nx - 1, j - 1)] + sol_old[idx(subdomain.nx - 1, j + 1)]) +
                                                     dx * dx * dy * dy * RHS[idx(subdomain.nx - 1, j)]) /
                                                    (2 * (dx * dx + dy * dy));
                    maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(subdomain.nx - 1, j)] - sol_old[idx(subdomain.nx - 1, j)]));
                }
            }
        }
        ...
        if (env.left_neighbor != MPI_PROC_NULL)
        {
            for (int i = 1; i < subdomain.nx - 1; ++i)
            {
                sol[idx(i, 0)] = (dy * dy * (sol_old[idx(i - 1, 0)] + sol_old[idx(i + 1, 0)]) +
                                  dx * dx * (received_left_boundary_data[i] + sol_old[idx(i, 1)]) +
                                  dx * dx * dy * dy * RHS[idx(i, 0)]) /
                                 (2 * (dx * dx + dy * dy));
                maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(i, 0)] - sol_old[idx(i, 0)]));
            }
        }
        ...
        if (env.right_neighbor != MPI_PROC_NULL)
        {
            for (int i = 1; i < subdomain.nx - 1; ++i)
            {
                sol[idx(i, subdomain.ny - 1)] = (dy * dy * (sol_old[idx(i - 1, subdomain.ny - 1)] + sol_old[idx(i + 1, subdomain.ny - 1)]) +
                                                 dx * dx * (sol_old[idx(i, subdomain.ny - 2)] + received_right_boundary_data[i]) +
                                                 dx * dx * dy * dy * RHS[idx(i, subdomain.ny - 1)]) /
                                                (2 * (dx * dx + dy * dy));
                maxAbsDiff = std::max(maxAbsDiff, std::abs(sol[idx(i, subdomain.ny - 1)] - sol_old[idx(i, subdomain.ny - 1)]));
            }
        }
